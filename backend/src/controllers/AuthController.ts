...
export class AuthController {
  ...
  public register = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (tokenError || !tokenData) {
        logger.error('Register - Token validation failed:', tokenError || 'No token data found');
      ...
      logger.info(`AuthController: Hashed password for registration (first 10 chars): ${hashedPassword.substring(0, 10)}`);
  ...
  public sendRegistrationVerificationCode = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      logger.info('SendCode - Saving new token:', { email, verificationToken, expiresAt: expiresAt.toISOString() });
      ...
      try {
        ...
        logger.error('SendCode - Error sending verification email:', emailError);
        throw createError('Failed to send verification email', 500);
      }

      if (tokenError) {
        logger.error('SendCode - Error creating new verification token:', tokenError);
  ...
  public async forgotPassword(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.info('AuthController: Received forgot password request for', email);
      ...
      logger.info('AuthController: Invalidating previous password reset tokens for user:', user.id);
      await supabase
        .from('password_reset_tokens')
        .update({ used: true, updated_at: new Date().toISOString() })
        .eq('user_id', user.id)
        .eq('used', false);
      logger.info('AuthController: Previous password reset tokens invalidated.');

      // Save new reset token to database
      logger.info('AuthController: Saving new password reset token:', { user_id: user.id, resetCode, expiresAt: expiresAt.toISOString() });
      const { error: tokenError } = await supabase
        .from('password_reset_tokens')
        .insert([
          {
      ...
      if (tokenError) {
        logger.error('AuthController: Error creating new password reset token:', tokenError);
      ...
      try {
        logger.info('AuthController: Before calling sendPasswordResetEmail for', user.email);
        await sendPasswordResetEmail(user.email, user.name, resetCode);
        logger.info('AuthController: After calling sendPasswordResetEmail for', user.email);
      } catch (emailError) {
        logger.error('AuthController: Failed to send reset email:', emailError);
        // Don't fail the request if email fails
        logger.error('AuthController: Email sending failed, but request will proceed.');
      }
  ...
  public resetPassword = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.info(`AuthController: resetPassword - Querying for token: ${code}, user_id: ${user.id}, used: false, expires_at >= ${new Date().toISOString()}`);
      logger.info('AuthController: resetPassword - Query result for token:', resetToken, 'Error:', tokenError);
      logger.info('AuthController: resetPassword - Current time for expiration check:', new Date().toISOString());

      if (resetToken) {
        logger.info(`AuthController: resetPassword - Found token in DB: ${resetToken.token}, expires_at: ${resetToken.expires_at}, used: ${resetToken.used}`);
        logger.info(`AuthController: resetPassword - Token expires_at: ${resetToken.expires_at}, Current time: ${new Date().toISOString()}, Token used: ${resetToken.used}`);
      }

      if (tokenError || !resetToken) {
        logger.error('AuthController: resetPassword - Token not found or invalid. tokenError:', tokenError, 'resetToken:', resetToken);
        logger.error(`AuthController: Código recibido en la solicitud: ${code}`);

        throw createError('Código inválido o expirado', 400);
      }
      logger.info('AuthController: resetPassword - Reset token found:', resetToken);
      logger.info(`AuthController: Código recibido en la solicitud: ${code}`);
      logger.info(`AuthController: Fecha de expiración del token: ${resetToken.expires_at}`);
